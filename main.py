...
import loading  # Import loading.py
...
def test_refresh():
    for i in range(4):  # Cycle numbers 0-9
...
# Function to transition to loading.py
import loading  # Ensure loading.py is in the same directory

def go_to_loading():
    clear_displays()
    if hasattr(loading, "run"):  # Check if function exists
        loading.run()
    else:
        print("Error: loading.py does not have a 'run' function.")
...
def main():
    ...
    test_refresh()
    clear_displays()
    go_to_loading()  # Transition to loading.py
